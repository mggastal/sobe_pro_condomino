#!/usr/bin/env python3
"""
Dashboard Pró Condomíno — Sobé Estratégias
Gera UM data.json com as duas filiais (CTBA e João Pessoa) para o
index.html standalone. O dash troca de cidade por um botão no topo.

Produção: lê das planilhas Google (gviz CSV).
Teste local: defina LOCAL_XLSX para apontar cada cidade a um .xlsx.
"""
...
# CONFIG
# ══════════════════════════════════════════════════════
# Uma entrada por filial. A KEY ("CTBA"/"JP") é a que o index.html usa.
CIDADES = {
    "CTBA": {
        "label":    "Curitiba",
        "sheet_id": "1Q00F93SEMPDmFLTJiooiM1YboRx42thAAJt-oK-L43g",
    },
    "JP": {
        "label":    "João Pessoa",
        "sheet_id": "1zGsNDmVxs7MvuTYLSob8AHRpU-qOIIXgRRoAvy7Dd2Y",
    },
}
CIDADE_PADRAO = "CTBA"          # filial mostrada ao abrir o dash

OUTPUT_JSON      = "data.json"

# Para gerar/testar offline a partir dos .xlsx em vez do Google Sheets,
# preencha { "CTBA": "ctba.xlsx", "JP": "jp.xlsx" }. Deixe vazio ({}) em produção.
LOCAL_XLSX       = {}
...
# ══ Fonte de dados (setado por build_city) ═════════════
# _SRC = ("gviz", sheet_id)  ou  ("xlsx", caminho.xlsx)
_SRC = ("gviz", CIDADES[CIDADE_PADRAO]["sheet_id"])

def read_tab(tab):
    """Lê uma aba pelo NOME, do Google Sheets (gviz) ou de um .xlsx local."""
    kind, ref = _SRC
    if kind == "xlsx":
        return pd.read_excel(ref, sheet_name=tab)
...
def sheet_url(t):
    _, sheet_id = _SRC
    return f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={t}"
...
def load_google():
    ...
    try:
        dfp=read_tab("google-ads-pesquisa")
    ...
    try:
        df2=read_tab("google-ads-outros")
...
def google_breakdowns(df):
    ...
    hoje=pd.Timestamp(date.today()); ontem=hoje-pd.Timedelta(days=1)
    try:
        df_a=read_tab("google-breakdown-age"); df_a["date"]=pd.to_datetime(df_a["Date (Segment)"],errors="coerce")
    ...
    try:
        df_g=read_tab("google-breakdown-gender"); df_g["date"]=pd.to_datetime(df_g["Date (Segment)"],errors="coerce")
...
def build_city(cidade_key, cfg, img_dir):
    """Processa UMA filial e devolve o dict de dados dela."""
    global _SRC, _dfp_pesquisa
    _dfp_pesquisa = pd.DataFrame()
    if cidade_key in LOCAL_XLSX:
        _SRC = ("xlsx", LOCAL_XLSX[cidade_key])
    else:
        _SRC = ("gviz", cfg["sheet_id"])

    print("="*60)
    print(f"Filial: {cidade_key} — {cfg['label']}  ({_SRC[0]})")
    print("="*60)
...
def main():
    ...
    saida = {"_META": {
        ...
        "cidades": {k: v["label"] for k, v in CIDADES.items()},
        "nome_cliente": NOME_CLIENTE,
    }}
    for cidade_key, cfg in CIDADES.items():
        try:
            saida[cidade_key] = build_city(cidade_key, cfg, img_dir)
        except Exception as e:
            print(f"  ✗ ERRO na filial {cidade_key}: {e}")
            raise
    ...
    print(f"\n✓ {OUTPUT_JSON} ({size}KB) — filiais: {', '.join(CIDADES.keys())}")
    print("="*60)
